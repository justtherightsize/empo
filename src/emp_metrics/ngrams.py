...
class SpanProcessor:
    ...
    def resume_spanify(self, data, pool_size=1):
        ...
        if not hashseed:
            raise RuntimeError("Python hash seed not set! Run with PYTHONHASHSEED=x python ...py ...")
        done = False
        while not done:
...
def evaluate(responses_filepath, col='gens', output_dir=None, write_cache_dir='.cache/'):

    results_df = pd.read_csv(responses_filepath, sep="~")
    results_df = results_df[results_df['gens'].str.len() != 0]
    print("num responses", len(results_df))
    ...
    start = time.time()
    _ = x.spanify(data, pool_size=15)
    end = time.time()
    print("Time took ", end - start)

    if not output_dir:
        system_name = responses_filepath.split('/')[-1] if col != 'gen_targets' else 'human'
        output_dir=os.path.join(OUTPUTS_HOME, f"ngrams/{system_name}")
...
def main():
    ...
    parser.add_argument(
        ...
        % os.path.join(p, "data/empathy_datasets/empathetic_dialogues", "outputs/ngrams"),
    )
    parser.add_argument(
        "--from-cache",
        default=None,
